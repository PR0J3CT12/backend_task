...
def drop_trades(request):
    ...
    return HttpResponse(json.dumps({'message': 'Таблица trades успешно очищена.'}, ensure_ascii=False))
...
def insert_into_trade(request):
    if request.body:
        ...
        return HttpResponse(json.dumps({'message': 'Body запроса пустое.'}, ensure_ascii=False), status=400)
    try:
        user = User.objects.get(id=request_body["user"])
        if request_body["type"] not in ["sell", "buy"]:
            return HttpResponse(json.dumps({'message': 'В поле type указан неверный тип. Необходимо указать "sell" или "buy".'}, ensure_ascii=False), status=400)
        ...
        return HttpResponse(json.dumps({'message': 'Пользователь не существует.'}, ensure_ascii=False), status=404)
    except ValueError:
        return HttpResponse(json.dumps({'message': 'В поле price указано неверное значение. Значение должно быть рациональным числом.'}, ensure_ascii=False), status=400)
    except DataError:
        return HttpResponse(json.dumps({'message': 'В поле symbol указано не верное значение. Длина введенного символа не должна превышать 4.'}, ensure_ascii=False), status=400)
    except KeyError as e:
        return HttpResponse(json.dumps({'message': f'Не указано поле {e}.'}, ensure_ascii=False), status=404)
    ...
    try:
        ...
        if user_id:
            ...
            for trade in trades_data:
                trade_json = {"type": trade.type, "user": {"id": user.id, "name": user.name}, "symbol": trade.symbol, "price": trade.price, "timestamp": str(trade.timestamp)}
            ...
            result_list = []
            for trade in trades_data:
                trade_json = {"type": trade.type, "symbol": trade.symbol, "price": trade.price, "timestamp": str(trade.timestamp)}
                result_list.append(trade_json)
            return HttpResponse(json.dumps(result_list, ensure_ascii=False))
    except ValidationError:
        return HttpResponse(json.dumps({'message': 'Пользователь не существует.'}, ensure_ascii=False), status=404)
...
def max_min_price(request, symbol):
    try:
        ...
        if len(trades_data_by_symbol) == 0:
            return HttpResponse(json.dumps({'message': f'Сделок с символом {symbol} не существует.'}, ensure_ascii=False), status=404)
        else:
            if len(trades_data_filtered) == 0:
                return HttpResponse(json.dumps({'message': f'Нет сделок в заданном периоде.'}, ensure_ascii=False), status=404)
        ...
        return HttpResponse(json.dumps({'message': 'В поле start или end указано неверное значение. Значение должно удовлетворять datetime формату: YYYY-MM-DD.'}, ensure_ascii=False), status=400)
